Syscoin Core version 0.18.0 is now available from:

  <https://syscoincore.org/bin/syscoin-core-0.18.0/>
...
`/Applications/Syscoin-Qt` (on Mac) or `syscoind`/`syscoin-qt` (on
Linux).
...
recommended to use Syscoin Core on unsupported systems.
...
`~syscoin/.syscoin`. When switching over to the new configuration file,
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/syscoin/syscoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://syscoincore.org/en/contact/) the Syscoin Core
...
`~syscoin/.syscoin`. This change makes Syscoin Core more consistent with
...
`datadir` option in `/etc/syscoin/syscoin.conf` will have no effect.
...
- A new short [document](https://github.com/syscoin/syscoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/syscoin/syscoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/syscoin/syscoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/syscoin/syscoin/blob/master/doc/syscoin-conf.md)
  about the `syscoin.conf` file describes how to use it to configure
  Syscoin Core.

- A new document introduces Syscoin Core's BIP174 [Partially-Signed
...
- The [output script
  descriptor](https://github.com/syscoin/syscoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Syscoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Syscoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/syscoin/syscoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/syscoin/syscoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Syscoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Syscoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/syscoin/syscoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/syscoin/syscoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- A new `syscoin-wallet` tool is now distributed alongside Syscoin
...
This section describes planned changes to Syscoin Core that may affect
other Syscoin software and services.

- Since version 0.16.0, Syscoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Syscoin Core 0.20 (expected about
  a year after 0.18), Syscoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Syscoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Syscoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Syscoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Syscoin Core
...
- When fetching a transaction announced by multiple peers, previous
  versions of Syscoin Core would sequentially attempt to download the
...
- A sub-project of Syscoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Syscoin Core.  See their [project
  page](https://github.com/syscoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Syscoin Core's own implementation, although entropy
  gathered by Syscoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Syscoin Core a
...
- On macOS, Syscoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `syscoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libsyscoin_server.a` (jnewbery)
...
- #13926 [Tools] syscoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14521 qt, docs: Fix `syscoin-qt -version` output formatting (hebasto)
...
- #11625 Add SyscoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libsyscoin_server` (laanwj)
...
- #15154 Configure: syscoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/syscoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Syscoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libsyscoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in syscoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Syscoin Core (Christewart)
...
- #12246 Bugfix: Only run syscoin-tx tests when syscoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_syscoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_syscoin` (MarcoFalke)
...
- #14186 syscoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_syscoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update syscoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/syscoin-conf.md (hebasto)
...
- #15354 Add missing `syscoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct syscoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support syscoin folder on OSX (JimmyMow)
