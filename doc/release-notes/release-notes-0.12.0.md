Syscoin Core version 0.12.0 is now available from:

  <https://syscoin.org/bin/syscoin-core-0.12.0/>
...
[libsecp256k1](https://github.com/syscoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/syscoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Syscoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Syscoin Core was to raise the default minimum
relay fee.

Syscoin Core 0.12 will have a strict maximum size on the mempool. The
...
Syscoin Core 0.12 also introduces new default policy limits on the length and
...
Syscoin Core 0.12 nodes. Syscoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/syscoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Syscoin Core 0.12 does not yet have support for
...
Syscoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Syscoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Syscoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Syscoin Core 0.13, and it is at this time undecided whether the
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Syscoin
Core to use floating fees. By default, Syscoin Core will use floating
fees.
...
At all times, Syscoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SYS.
Furthermore, Syscoin Core will never create transactions paying less than
...
`syscoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/syscoin/bips/blob/master/bip-0065.mediawiki)
...
Restarting syscoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Syscoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new syscoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include syscoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix syscoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of syscoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override SYSCOIND and SYSCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [syscoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add SYSCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libsyscoinconsensus (Braydon Fuller)
